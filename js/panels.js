// Paneles del lateral: se pueden plegar y desanclar como ventanas flotantes redimensionables.
...
export function initPanels(sidebar) {
  ...
  for (const sec of panels) {
    ...
    h2.addEventListener('pointerdown', (e) => {
      ...
      const move = (ev) => { cfg.x = x0 + ev.clientX - sx; cfg.y = y0 + ev.clientY - sy; place(); };
      const up = () => { h2.removeEventListener('pointermove', move); h2.removeEventListener('pointerup', up); save(st); };
    ...
    api[id] = { float, dock, setCollapsed, el: sec };
  }
