...
/** Puente (de r.bridges) que contiene la posición s, o null. */
function bridgeAt(r, sv) {
...
export function buildEdgeMeshes(layout, elev, spIn = {}, opts = {}) {
  ...
  layout.routes.forEach((r, k) => {
    ...
    const brF = frames.map((F) => bridgeAt(r, F.s)); // puentes: camino de tierra y barrera propios (a ambos lados)
    for (const side of [1, -1]) {
      ...
      if (dAt.some((w) => w > 0)) {
        ...
        for (const [gk, grp] of Object.entries(G)) {
          if (!grp.idx.length) continue;
          const bi = gk[0] === 'b' ? +gk.slice(1) : null;
          const name = bi != null ? `camino_tierra_${tyOf(bi)}_${String(bi + 1).padStart(2, '0')}_${sideName}` : gk === 's' ? `camino_tierra_${r.name}_suspendido_${sideName}` : `camino_tierra_${r.name}_${sideName}`;
          res.dirt.push({ name, k, side, alt: isAlt, susp: gk === 's', bridge: bi, bridgeTy: bi != null ? tyOf(bi) : null, positions: P32, uvs: U32, indices: grp.idx, segs: grp.segs, per });
          res.dirtTris += grp.idx.length / 3;
        }
      }
      if (bOn.some(Boolean)) {
        ...
        frames.forEach((F, a) => {
          ...
          const sk = suspF[a] || brF[a] ? 0 : skirt;
        ...
        for (const [gk, grp] of Object.entries(G)) {
          if (!grp.idx.length) continue;
          const bi = gk[0] === 'b' ? +gk.slice(1) : null;
          const name = bi != null ? `barrera_${tyOf(bi)}_${String(bi + 1).padStart(2, '0')}_${sideName}` : gk === 's' ? `barrera_${r.name}_suspendido_${sideName}` : `barrera_${r.name}_${sideName}`;
          res.barriers.push({ name, k, side, alt: isAlt, plane, susp: gk === 's', bridge: bi, bridgeTy: bi != null ? tyOf(bi) : null, positions: P32, uvs: U32, indices: grp.idx, segs: grp.segs, per: 6, s: sArr });
          res.barrierTris += grp.idx.length / 3;
        }
      }
    }
  });
